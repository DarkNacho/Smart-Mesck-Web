...
export default class ObservationService extends FhirResourceService<Observation> {
  ...
  public getValue(observation: Observation) {
    if (observation.valueQuantity) {
      return `${observation.valueQuantity.value}${observation.valueQuantity.code ? ` ${observation.valueQuantity.code}` : ""}`;
    }
  
    if (observation.valueCodeableConcept) {
      return observation.valueCodeableConcept.text || "Unknown Value CodeableConcept";
    }
    ...
    if (observation.valueRange) {
      ...
      return low !== undefined && high !== undefined ? `${low} - ${high} ${unit}` : "Unknown ValueRange";
    }
    ...
    if (observation.valueRatio) {
      ...
      return numerator !== undefined && denominator !== undefined ? `${numerator} / ${denominator}` : "Unknown ValueRatio";
    }
